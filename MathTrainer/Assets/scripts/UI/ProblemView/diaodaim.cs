...
public class diaodaim : MonoBehaviour
{
    ...
    public void Answer(){
        if(problem.InputAnswer(AnswerText.text == "" ? new string[0] : AnswerText.text.Split('|'))){
            WhenCorrect();
        }else{
            WhenWrong();
        }
    ...
    void WhenWrong(){
        if(problem.Answers.Length == 0){
            DescriptionText.text = "回答错误，该题答案为无解";
        }
        DescriptionText.text = "回答错误，答案如下\n";
        for(int i = 0;i<problem.Answers.Length;i++){
            DescriptionText.text += $"答案项{i+1}为：{problem.Answers[i]}\n";
        }
    }
    public void Click()
    {
